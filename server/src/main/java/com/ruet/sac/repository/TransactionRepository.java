...
public interface TransactionRepository extends JpaRepository<Transaction, String> {
    ...
    @Query("Select t.id,t.reciepentBankAccountNo,t.bankName,t.status,tc.id,tc.name,tc.picture,e.eventName from Transaction t join t.transactionCreator tc join t.event e where e.id=1 and ( t.id like %:searchText% or t.reciepentBankAccountNo like %:searchText% or t.bankName like %:searchText% or cast(tc.id as string) like %:searchText% or tc.name like %:searchText% or e.eventName like %:searchText%) order by t.id desc")
    List<Object[]> getAllFilteredPendingAlumniRegistration(String searchText);

    @Query("Select t.id,t.reciepentBankAccountNo,t.bankName,t.status,t.bankReceipt,tc.id,tc.name,tc.series,tc.picture,tc.contactNo,e.id,e.eventName,e.eventDate from Transaction t join t.transactionCreator tc join t.event e where t.id=:transactionId  order by t.id desc")
    Object[] getPendingEventRegistration(String transactionId);
}