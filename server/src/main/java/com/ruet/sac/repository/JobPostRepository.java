...
public interface JobPostRepository extends JpaRepository<JobPost, Integer> {
    ...
    @Query("Select p.id,p.title,p.companyName,p.location,p.date,p.deadline,p.durationType,p.placementType,p.description,p.applicationlink,pw.id,pw.name,pw.picture"
            ...
            +" where (p.title like %:searchText% or p.companyName like %:searchText% or p.location like %:searchText% or p.description like %:searchText%) and (p.durationType like %:durationType%) and p.placementType like %:placementType% and p.deadline like %:deadline% "
